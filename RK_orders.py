...
ordering = " ≫ ".join(name for name, _ in sorted_errors)

if ordering == "error_Euler ≫ error_Heun ≫ error_RK4":
    st.success(f"Observed ordering: {ordering}")
else:
    st.warning(
        f"Error ordering differs for this choice of λ and h and is: {ordering}"
    )